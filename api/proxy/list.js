module.exports = async function handler(req, res) {
  ...
  const protocol = (req.query && req.query.protocol) || 'http,https,socks5,socks4';
  let proxies = [];
  ...
  const tryGeoNode = async () => {
    ...
    return (d.data || []).map(p => ({ ip: p.ip, port: p.port, protocols: p.protocols || ['http'], country: p.country || '' }));
  };
  ...
  if (!proxies.length) {
    for (const proto of protos) { try { proxies.push(...(await tryProxyScrape(proto))); } catch {} }
  }
